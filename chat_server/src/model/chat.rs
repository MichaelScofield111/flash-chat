use crate::model::{ChatType, ChatUser};
use crate::{AppError, model::Chat};
    ...
    pub async fn create(input: CreateChat, ws_id: u64, pool: &PgPool) -> Result<Self, AppError> {
        ...
        if len < 2 {
            return Err(AppError::CreateChatError(
                "Chat must ba have 2 or more members".to_string(),
            ));
        }

        if len > 8 && input.name.is_none() {
            return Err(AppError::CreateChatError(
                "Group chat  with more than 8 members must be have a name".to_string(),
            ));
        }
        ...
        let chat = sqlx::query_as(
            r#"
                ...
                VALUES ($1, $2, $3, $4)
                RETURNING id, name, type, members, ws_id, created_at
                "#,
        )
        .bind(input.name)
        .bind(chat_type)
